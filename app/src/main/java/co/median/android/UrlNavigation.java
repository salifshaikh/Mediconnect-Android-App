    ...
    private void initializeCaptureIntentLauncher() {
        this.captureIntentLauncher = mainActivity.registerForActivityResult(
                ...
                result -> {
                    ...
                    // for video recording
                    if (this.creator.getCurrentVideoRecordingUri() != null) {
                        if (uploadCallback != null) {
                            uploadCallback.onReceiveValue(new Uri[]{this.creator.getCurrentVideoRecordingUri()});
                            uploadCallback = null;
                        }
                        return;
                    }

                    // from documents (and video)
                    ...
                    if (this.creator.getCurrentCaptureUri() != null) {
                        ...
                        Uri captureUri = this.creator.getCurrentCaptureUri();

                        if (AppConfig.getInstance(mainActivity).cameraConfig.getCaptureQuality() == CaptureQuality.LOW) {
                            MediaFileHelper.resizeJpgUriTo480p(mainActivity, captureUri, new FileCallback() {
                                @Override
                        ...
                        if (uploadCallback != null) {
                            uploadCallback.onReceiveValue(new Uri[]{captureUri});
                            uploadCallback = null;
                        }
